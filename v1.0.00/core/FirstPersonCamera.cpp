...
void FirstPersonCamera::processKeyboard(bool keys[], float deltaTime) {
    ...
    if (glm::length(move) > 0.0f) {
        ...
        // Player is actively moving – override velocity
        player->getRigidBody()->velocity.x = move.x * speed;
        player->getRigidBody()->velocity.z = move.z * speed;
    } else {
        // No input – do NOT zero velocity! Only apply ground friction if grounded.
        if (player->getRigidBody()->isGrounded) {
            player->getRigidBody()->velocity.x *= 0.8f; // simple friction
            player->getRigidBody()->velocity.z *= 0.8f;
            // Stop completely if very slow
            if (fabs(player->getRigidBody()->velocity.x) < 0.1f) player->getRigidBody()->velocity.x = 0.0f;
            if (fabs(player->getRigidBody()->velocity.z) < 0.1f) player->getRigidBody()->velocity.z = 0.0f;
        }
    }
}
